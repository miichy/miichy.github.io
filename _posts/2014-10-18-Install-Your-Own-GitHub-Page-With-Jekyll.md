...
tags : [github, beginner, jekyll]
---
{% include JB/setup %}

Github Page可谓是程序猿装13利器，又因最近决定一周最少一次记录日常生活，碍于没有一个技术网站的博客是我喜欢的类型，所以决定自己建一个github page。经历了两个小时的折腾，终于在这里有一个自己的小站啦。现在记录一下花了两个小时所经历的。

## Overview

### 创建域名创库
简单而言就是注册github账号，然后按照page.github.com的help进行。这里面有github自带的页面可以使用，就不引用了。

### 克隆创库

使用git命令将上一步骤的创库拉到本地。window用户在这一步需要安装github的客户端，其实用git_bash工具也可以，但是这个工具在生成密钥这一步就不能起作用了。


### 安装Jekyll

这一步是最费时间最坑的。由于我本级的是ruby2.0，因版本太低，只能升级到2.1.3才能使用。其中到一个错误是“Failed to build gem native extension”，有网友说是没有安装gcc，有网友说是没有update rvm。最终还是因为版本问题。

### 使用jekyll

**Jekyll is a parsing engine.**

使用jekyll new -f miichy.github.io，在进入该目录，执行jekell server命令。如果 0.0.0.0:4000出现了正确页面。关闭jekyll服务，git push code即可。
